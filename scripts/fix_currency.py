...
def fix_currency_in_file(filepath, is_price_file=False):
    """修复文件中的货币显示"""
    ...
    if is_price_file and 'stocks' in data:
        # 修复latest_price.json
        ...
        for pick in data['picks']:
            if 'currency' in pick:
                ...
                fixed_count += 1
            # 确保没有SGD相关文本
            if 'potential_reasons' in pick:
                pick['potential_reasons'] = pick['potential_reasons'].replace('SGD', 'RM')
    
    # 保存修复后的文件
    with open(filepath, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    
    print(f"✅ 修复了 {fixed_count} 个货币字段")
    return True

def fix_all_currency():
    """修复所有JSON文件中的货币"""
    web_dir = "../web"
    
    # 1. 修复picks_latest.json
    picks_file = os.path.join(web_dir, "picks_latest.json")
    fix_currency_in_file(picks_file)
    
    # 2. 修复latest_price.json
    price_file = os.path.join(web_dir, "latest_price.json")
    fix_currency_in_file(price_file, is_price_file=True)
    ...
    for filename in os.listdir(web_dir):
        if filename.startswith('picks_') and filename.endswith('.json'):
            ...
            fix_currency_in_file(filepath)
    
    # 4. 创建马来西亚特定的股票数据处理函数
    create_malaysia_stock_processor()
    
    return True

def create_malaysia_stock_processor():
    """创建马来西亚股票数据处理器"""
    print("\n🇲🇾 创建马来西亚股票数据处理脚本...")
...
'''
    ...
    processor_file = os.path.join("../web", "bursa_processor.py")
    with open(processor_file, 'w', encoding='utf-8') as f:
        f.write(processor_content)
    
    print(f"✅ 创建马来西亚股票处理器: {processor_file}")
    
    # 运行处理器
    print("\n🚀 运行马来西亚股票处理器...")
    os.system(f"cd ../web && python3 bursa_processor.py")
