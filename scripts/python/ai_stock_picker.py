...
def enhance_stock_data(df):
    ...
    def auto_detect_sector(row):
        ...
        # 如果已经有合理的行业名称，直接使用
        if sector and sector.lower() not in ['unknown', '0', 'nan', '']:
            # 检查是否是数字代码
            if sector.isdigit():
                # 数字代码转换为标准行业名称
                return convert_sector_code(sector)
            return sector
        
        # 基于股票名称识别
        name_for_detection = name or code
        
        # 按C后缀判断凭单
        if any(x in name_for_detection for x in ['-c', '-wa', '-wb', '-wr']):
            return 'Warrants'
        
        # 基于关键词识别
        sector_keywords = {
            'tech': ['tech', 'tronics', 'digital', 'software'],
            'finance': ['bank', 'finance', 'capital', 'insurance'],
            'industrial': ['industr', 'manufact', 'engineer'],
            'construction': ['construct', 'build', 'develop'],
            'plantation': ['plant', 'palm', 'rubber'],
            'property': ['prop', 'realty', 'estate'],
            'consumer': ['consumer', 'retail'],
            'healthcare': ['health', 'medical', 'pharma'],
            'transport': ['transport', 'logistic'],
            'telco': ['telecom', 'telco', 'maxis', 'digi'],
            'utilities': ['utility', 'power', 'tenaga'],
            'energy': ['petrol', 'oil', 'gas'],
            'food': ['food', 'nestle', 'beverage'],
        }
        
        for sector_name, keywords in sector_keywords.items():
            for keyword in keywords:
                if keyword in name_for_detection:
                    return sector_name.capitalize()
        
        return 'Other'
    
    # 数字代码转换函数
    def convert_sector_code(code):
        """转换行业数字代码"""
        if not code.isdigit():
            return code
    ...
    # 应用行业识别
    df['sector'] = df.apply(auto_detect_sector, axis=1)
...
def calculate_ai_scores(df):
    ...
    for _, row in df.iterrows():
        try:
            ...
            # 嘗試獲取價格數據
            price_columns = ['close', 'last', 'price', 'closing']
            price = None
            ...
            if price is None:
                price = np.random.uniform(0.1, 15.0)  # 隨機價格作為備用
            
            # 計算模擬技術指標
            momentum = np.random.uniform(-10, 15)  # -10% 到 +15%
            rsi = np.random.uniform(30, 70)       # 30-70之間
            volume_ratio = np.random.uniform(0.5, 3.0)
            ma_ratio = np.random.uniform(0.8, 1.2)
