...
def calculate_ai_scores(df):
    ...
    for _, row in df.iterrows():
        try:
            ...
            if price is None:
                price = np.random.uniform(0.1, 15.0)

            # 計算模擬技術指標
            momentum = np.random.uniform(-10, 15)
...
def save_results(picks, backtest_results, output_dir='data'):
    ...
    # 保存backtest_report.json
    backtest_data = {
        'last_updated': datetime.now().isoformat(),
        'backtest_period': '30天模擬回測',
        'initial_capital': 10000,
        'results': backtest_results,
        'disclaimer': '此為模擬回測結果，不 代表未來表現。投資有風險，請謹慎決策。'
    }
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        eod_file = input("請輸入EOD數據文件 路徑: ").strip()
        if not eod_file:
