...
def main():
    ...
    p_lookup = config.get("sector_lookup", {})
    ...
    for r in rows[1:]:
        ...
        sector_code = normalize_code(record.get("Sector Code")) if record.get("Sector Code") else code

        # 查找行業
        sector = None
        if sector_code:
            sector = p_lookup.get(sector_code)
            if not sector:
                # 後備方案：只保留數字
                digits_only = "".join(ch for ch in sector_code if ch.isdigit())
                sector = p_lookup.get(digits_only, "Unknown")
        else:
            sector = "Unknown"
        ...
        # 記錄未找到的行業代碼
        if sector == "Unknown" and sector_code:
