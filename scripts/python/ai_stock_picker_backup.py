...
def enhance_stock_data(df):
    ...
    # 清理Excel公式格式（例如：="7117" -> 7117）
    def clean_excel_formula(value):
        if isinstance(value, str):
            # 移除 ="" 包围的格式
            if value.startswith('="') and value.endswith('"'):
                return value[2:-1]  # 移除 = 和 ""
            # 移除 = 开头
    ...
    if 'code' not in df.columns and len(df.columns) > 0:
        ...
        df['code'] = df[first_col].apply(clean_excel_formula).astype(str).str.strip()
    ...
    for idx, row in df.iterrows():
        ...
        for keyword, sector in sector_keywords.items():
            if keyword.lower() in str(row.get('name', '')).lower():
                df.at[idx, 'sector'] = sector
                sector_found = True
                break

        # 如果是憑單
        if not sector_found and any(x in str(row['code']) for x in ['-C', '-WA', '-WB']):
            df.at[idx, 'sector'] = 'Warrants'
